...
class WaitingWellsController < ApplicationController # :nodoc:
  def new
    @facade = WaitingWells::NewFacade.new
  ...
  def create
    ...
    if verified?(@referral)
      ReferralMailer.new_waiting_well(@referral).deliver_now
      redirect_to({ action: :new }, notice: 'Thank you for contacting the Live Well team')
    else
      render :new
    end
  end
